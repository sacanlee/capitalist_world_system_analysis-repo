# -*- coding: utf-8 -*-
"""2010年地图：七类着色（后工业化/成熟/中后期/中期/起步/准备 + 其他<1000万），
配色与 2024/2040/2055/1995 各图一致。发达国家（IMF口径）默认列为后工业化国家。"""
import csv
import json
import os
...
ax.set_aspect("auto")
ax.axis("off")
ax.set_title("全球工业化阶段地图（2010年，按人均发电量与人均钢产量标准划分）",
             fontsize=19, pad=8)
...
sub_y = bb.y0 - 0.010
fig.text(0.5, sub_y, "人口1000万及以上国家（与2024/2040/2055/1995各图同一国家名单） · 2010年数据",
...
lines = [
    ...
    "3. 归类顺序：发达国家（IMF 发达经济体口径）默认列为后工业化国家（以色列、中国台湾地区按此列入）；其余国家从高标准到低标准依次对照，",
    "　　以所能达到的最高阶段归类；四档均未达到者（起步档得分 < 2）列为准备工业化国家。中国按记分（中后期档得分 2.61）归入工业化中后期国家。",
    "4. 数据来源：人口 = 联合国 WPP2024（2010年历史值，全球 70.22 亿）；发电量 = Ember（OWID 整理）2010年；粗钢产量 = 世界钢铁协会《Steel Statistical Yearbook 2012》2010年",
    "　　（未列入其国别表者按无产量计；解析的国家合计 14.32 亿吨，与该协会公布的世界产量 14.33 亿吨一致）。",
]
for t in lines:
...
g = {}
for r in fin.values():
    g.setdefault(r["category"], []).append(float(r["pop2010"]))
tot = sum(v for vs in g.values() for v in vs)
uncls = WORLD_POP - tot
rows = [(c, len(g[c]), sum(g[c])) for c in ORDER]

y -= 0.004
fig.text(0.03, y, "各阶段人口（2010年）", fontsize=15, weight="bold")
y -= 0.028
...
y -= 0.024
notes = [
    "注：本图国家名单与2024年图一致（2024年人口≥1000万的95国 + 以色列），1995/2010两图中人口不足1000万的国家（阿联酋等）仍按记分/发达口径着色，其余未分类国家以浅紫色显示；",
    "　　南苏丹2011年独立，2010年其电力与钢铁统计计入苏丹，本图按0计并列为准备工业化国家。",
    "　　临界国家（得分2.0±0.2）：中国1.96、哈萨克斯坦1.85处于成熟档边界；南非2.14、波兰2.07、马来西亚2.12处于中后期档边界；乌兹别克斯坦2.08、叙利亚2.10、阿塞拜疆2.17、",
    "　　塔吉克斯坦2.15处于中期档边界；朝鲜2.09处于起步档边界。后工业化国家中，英国（成熟档1.90）、希腊（1.68）、葡萄牙（1.62）按记分不足成熟档，系按发达国家默认档列入。",
]
for t in notes:
    fig.text(0.03, y, t, fontsize=9.6, color="#666666")
    y -= 0.018

fig.savefig(os.path.join(OUT, "world_industrial_stage_map_2010.png"), facecolor="white")
print("saved out/world_industrial_stage_map_2010.png")
for c, n, p in rows:
